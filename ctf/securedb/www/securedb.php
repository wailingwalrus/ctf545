...
if (isset($_GET['method']))
{
	switch ($_GET['method'])
	{
		...
		case 'storeProtectedRecord':
			storeProtectedRecord(sanitize($_GET['fileName']), sanitize($_GET['password']), sanitize($_GET['name']), sanitize($_GET['ssn']));
			break;
		case 'userAccessField':
			userAccessField(${sanitize($_GET['fileName'])}, sanitize($_GET['key']), sanitize($_GET['password']));
			break;
		default:
